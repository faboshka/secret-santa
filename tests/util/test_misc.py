...
def test_is_derangement_valid_input[T](list1: list[T], list2: list[T], is_derangement: bool) -> None:
...
def test_is_derangement_fails_on_empty_list[T](list1: list[T], list2: list[T]) -> None:
    with pytest.raises(AssertionError) as exception_info:
        misc.is_derangement(list1, list2)
    assert "The list must not be empty to qualify for a derangement check" in str(exception_info.value), (
...
def test_is_derangement_fails_on_lists_of_different_length[T](list1: list[T], list2: list[T]) -> None:
    with pytest.raises(AssertionError) as exception_info:
        misc.is_derangement(list1, list2)
...
def test_is_derangement_fails_on_lists_of_different_types[T](list1: list[T], list2: list[T]) -> None:
    with pytest.raises(AssertionError) as exception_info:
        misc.is_derangement(list1, list2)
